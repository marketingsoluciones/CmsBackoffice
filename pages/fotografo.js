...
const componentMap = {
    ...
    Proyectos: Proyectos,
    FotografoInfoPage: FotografoInfoPage,
}
...
const FotografoPage = () => {
    ...
    const dataComponents = useMemo(() => {
        ...
        return fotografoSchema.subComponents
            .map((sub, originalIdx) => {
    ...
    const currentComponentElement = useMemo(() => {
        ...
        if (current.componentName === "Colecciones") {
            return <Component setComponentState={setOptionSelect} />
        } else if (current.componentName === "Proyectos") {
            return <Component setComponentState={setOptionSelect} />
        } else if (current.componentName === "FotografoInfoPage") {
    ...
    useEffect(() => {
        ...
        // Por defecto, mostrar FotografoInfoPage si existe, sino el primero
        const infoIndex = dataComponents.findIndex(item => item.componentName === "FotografoInfoPage")
        if (infoIndex !== -1) {
